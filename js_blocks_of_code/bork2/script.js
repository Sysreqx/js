var n = 999, //кол-во снежинок
    speed = 2, // скорость
    wind = 0, // наклон снежинок
    windVariance = 1.5; // отклонение снежинок, 1 = без отклонения
    if (window.innerWidth <= 1024) {
        c = document.getElementById("bg-img2");
        p = document.getElementById("secondSnowfall");
        n = 455;
    } else {
        c = document.getElementById("bg-img");
        p = document.getElementById("firstSnowfall");
    }
ctx = c.getContext("2d");
    cw = (c.width = p.offsetWidth);
ch = (c.height = p.offsetHeight);
img = new Image(64,64);
    particles = [];
    particleNumber = 0;
